...
@onready var safe_shape_2d: CollisionShape2D = $SafeShape2D
@onready var danger_shape_2d: CollisionShape2D = $DangerShape2D
@onready var danger_shape_2d2: CollisionShape2D = $DangerShape2D2
...
func _on_body_shape_entered(_body_rid: RID, body: Node2D, _body_shape_index: int, local_shape_index: int) -> void:
	...
	if hit_shape_node == collision_shape_2d and alive:
		if body.name == "Player" and body.alive:
			...
			direction *= -1
			animated_sprite_2d.flip_h = !animated_sprite_2d.flip_h
	elif hit_shape_node == vulnerable_shape_2d:
		if body.name == "Player" and body.alive:
			alive = false
			body.jump_boost()
